...
// with multiple attention head, need a new layout
//typedef struct dci_multi {
//	struct dci* dci_list;
//} dci_multi;

typedef struct dci {
	int num_heads;			// Number of attention head
	...
	int num_points;
	idx_elem* indices; // Assuming row-major layout, matrix of size num_heads x ( required_num_points x (num_comp_indices*num_simp_indices))
...
void dci_gen_proj_vec(float* proj_vec, const int num_heads, const int dim,
		const int num_indices);

void dci_init(dci* const dci_inst, const int num_heads, const int dim,
		const int num_comp_indices, const int num_simp_indices, 
		const int devId);
